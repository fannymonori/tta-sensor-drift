...
def get_random_subset_array(X, Y, one_hot_encoder, subset_size=4):
    ...
    tf.random.set_seed(ts)
    tf.keras.utils.set_random_seed(ts)
...
def run_pseudolabeling_TA(train_time_model, data, model_online_student, model_online_teacher, path):
    ## FOR VERGARA DRIFT DATASET
    ###################### Online retraining with student-teacher method
    ...
    rpx, _ = C_replay_memory.getMemoryAsArray()
    ...
    for k, v in test_batches.items():
        ...
        for batch_X, batch_Y in zip(batches_X, batches_Y):
            ...
            labels_logits = model_online_teacher(batch_X, training=False)
            ...
            #online_training_batch_X = np.concatenate((replay_memory_X, subset_X), axis=0)
            #online_training_batch_X = replay_memory_X
            #online_training_batch_X = shuffle(online_training_batch_X)
            #online_training_batch_X = batch_X
            ...
            with tf.GradientTape() as tape:
                logits_student = model_online_student(online_training_batch_X, training=True)
                logits_teacher = model_online_teacher(online_training_batch_X, training=False)
            ...
            moving_acc_online = evaluate(batch_X, batch_Y, model_online_teacher)
    ...
    print("Finished student-teacher batch learning approach")
